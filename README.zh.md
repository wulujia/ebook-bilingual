...
- **目录补救**:源 EPUB 若没有目录(或目录过于简陋),就从各章的 `<h1>`/`<h2>` 标题生成一份扁平
  目录,让被拆成多章却无法跳转的书重新可导航。本来就有完整目录的书保持原样。
...
- **目录生成**:打包时,目录缺失或过简(≤ 1 条)的 EPUB 会据各章首个 `<h1>`/`<h2>` 生成 EPUB3
  nav 文档**和** EPUB2 NCX(兼容旧阅读器)。保守:已有的多条目录绝不覆盖;`--no-toc` 可关闭。
...
- **PDF 分章**是 best-effort(识别显式「Chapter N」/ 全大写标题);漏了也不影响,全书会作为
  一整篇正常阅读。
- **自动目录依赖标题**——目录补救靠 `<h1>`/`<h2>` 识别章节,所以整本只有一个文件、或各章没有
  h1/h2 标题的书不会自动生成目录(但仍可从头读到尾)。已有的完整目录保留原标题。
